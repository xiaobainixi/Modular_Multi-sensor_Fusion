...
VinsFeatureManager::VinsFeatureManager(
    const std::shared_ptr<Parameter> &param_ptr,
    const std::shared_ptr<Viewer> &viewer_ptr)
{
    param_ptr_ = param_ptr;
    viewer_ptr_ = viewer_ptr;
}

void VinsFeatureManager::clearState()
{
    feature.clear();
}
...
int VinsFeatureManager::getFeatureCount()
{
    ...
    for (auto &it : feature)
    {
        ...
        if (
            it.used_num >= 2 &&
            it.start_frame < param_ptr_->WINDOW_SIZE - 2)
        {
            cnt++;
        }
    }
...
void VinsFeatureManager::triangulate()
{
    ...
    for (auto &it_per_id : feature) {
        ...
        CHECK(!it_per_id.feature_per_frame.empty());
        auto &first_frame = it_per_id.feature_per_frame[0];
        CHECK(first_frame.state_ptr_);
        Eigen::Matrix3d Rwb0 = first_frame.state_ptr_->Rwb_.toRotationMatrix();
        Eigen::Vector3d twb0 = first_frame.state_ptr_->twb_;
        Eigen::Matrix3d R0 = Rwb0 * param_ptr_->Rbc_;
        Eigen::Vector3d t0 = twb0 + Rwb0 * param_ptr_->tbc_;

        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            imu_j++;
            CHECK(it_per_frame.state_ptr_);
            Eigen::Matrix3d Rwb = it_per_frame.state_ptr_->Rwb_.toRotationMatrix();
            Eigen::Vector3d twb = it_per_frame.state_ptr_->twb_;
            Eigen::Matrix3d R1 = Rwb * param_ptr_->Rbc_;
            Eigen::Vector3d t1 = twb + Rwb * param_ptr_->tbc_;
...
void VinsFeatureManager::removeOutlier()
{
    LOG(FATAL) << "removeOutlier called: breaking intentionally";
    int i = -1;
    for (auto it = feature.begin(), it_next = feature.begin();
         it != feature.end(); it = it_next)
    {
...
void VinsFeatureManager::removeBackShiftDepth(Eigen::Matrix3d marg_R, Eigen::Vector3d marg_P, Eigen::Matrix3d new_R, Eigen::Vector3d new_P)
{
         ...
         it != feature.end(); it = it_next)
    {
        ...
        /*
        if (it->endFrame() < param_ptr_->WINDOW_SIZE - 1)
        {
            feature.erase(it);
        }
        */
...
// 对margin倒数第二帧进行处理
void VinsFeatureManager::removeFront(int frame_count)
{
