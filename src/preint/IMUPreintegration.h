...
public:
        ...
        const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        Eigen::Quaterniond corrected_delta_q = delta_q_ * Converter::deltaQ(dq_dbg * dbg);
        ...
        const T *const Baj, const T *const Bgj, T *residuals) const
    {
        ...
        Eigen::Quaternion<T> corrected_delta_q = delta_q_.cast<T>();

        Eigen::Matrix<T, 3, 1> corrected_delta_v = delta_v_.template cast<T>();
        Eigen::Matrix<T, 3, 1> corrected_delta_p = delta_p_.template cast<T>();
...
//             if (jacobians[0])
//             {
//                 Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> jac(jacobians[0]);
//                 jac.setZero();
//                 jac.block<3,3>(param_ptr_->POSI_INDEX, 0) = -Qi.inverse().toRotationMatrix();
//                 jac = sqrt_info * jac;
//             }
//             // 1: Qi
//             if (jacobians[1])
//             {
//                 Eigen::Map<Eigen::Matrix<double, 15, 4, Eigen::RowMajor>> jac(jacobians[1]);
//                 jac.setZero();
//                 jac.block<3,3>(param_ptr_->POSI_INDEX, 0) = Converter::Skew(Qi.inverse() * (-0.5 * param_ptr_->gw_ * sum_dt * sum_dt + Pj - Pi - Vi * sum_dt));
//                 Eigen::Quaterniond corrected_delta_q = preint_->delta_q_ * Converter::deltaQ(dq_dbg * (Bgi - preint_->bg_));
//                 jac.block<3,3>(param_ptr_->ORI_INDEX_STATE_, 0) = -(Converter::Qleft(Qj.inverse() * Qi) * Converter::Qright(corrected_delta_q)).bottomRightCorner<3,3>();
//                 jac.block<3,3>(param_ptr_->VEL_INDEX_STATE_, 0) = Converter::Skew(Qi.inverse() * (-param_ptr_->gw_ * sum_dt + Vj - Vi));
//                 jac = sqrt_info * jac;
//             }
//             // 2: Vi
//             if (jacobians[2])
//             {
//                 Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> jac(jacobians[2]);
//                 jac.setZero();
//                 jac.block<3,3>(param_ptr_->POSI_INDEX, 0) = -Qi.inverse().toRotationMatrix() * sum_dt;
//                 jac.block<3,3>(param_ptr_->VEL_INDEX_STATE_, 0) = -Qi.inverse().toRotationMatrix();
//                 jac = sqrt_info * jac;
//             }
//             // 3: Bai
//             if (jacobians[3])
//             {
//                 Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> jac(jacobians[3]);
//                 jac.setZero();
//                 jac.block<3,3>(param_ptr_->POSI_INDEX, 0) = -dp_dba;
//                 jac.block<3,3>(param_ptr_->VEL_INDEX_STATE_, 0) = -dv_dba;
//                 jac.block<3,3>(param_ptr_->ACC_BIAS_INDEX_STATE_, 0) = -Eigen::Matrix3d::Identity();
//                 jac = sqrt_info * jac;
//             }
//             // 4: Bgi
//             if (jacobians[4])
//             {
//                 Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> jac(jacobians[4]);
//                 jac.setZero();
//                 jac.block<3,3>(param_ptr_->POSI_INDEX, 0) = -dp_dbg;
//                 jac.block<3,3>(param_ptr_->ORI_INDEX_STATE_, 0) = -Converter::Qleft(Qj.inverse() * Qi * preint_->delta_q_).bottomRightCorner<3,3>() * dq_dbg;
//                 jac.block<3,3>(param_ptr_->VEL_INDEX_STATE_, 0) = -dv_dbg;
//                 jac.block<3,3>(param_ptr_->GYRO_BIAS_INDEX_STATE_, 0) = -Eigen::Matrix3d::Identity();
//                 jac = sqrt_info * jac;
//             }
//             // 5: Pj
//             if (jacobians[5])
//             {
//                 Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> jac(jacobians[5]);
//                 jac.setZero();
//                 jac.block<3,3>(param_ptr_->POSI_INDEX, 0) = Qi.inverse().toRotationMatrix();
//                 jac = sqrt_info * jac;
//             }
...
//             if (jacobians[6])
//             {
//                 Eigen::Map<Eigen::Matrix<double, 15, 4, Eigen::RowMajor>> jac(jacobians[6]);
//                 jac.setZero();
//                 Eigen::Quaterniond corrected_delta_q = preint_->delta_q_ * Converter::deltaQ(dq_dbg * (Bgi - preint_->bg_));
//                 jac.block<3,3>(param_ptr_->ORI_INDEX_STATE_, 0) = Converter::Qleft(corrected_delta_q.inverse() * Qi.inverse() * Qj).bottomRightCorner<3,3>();
//                 jac = sqrt_info * jac;
//             }
//             // 7: Vj
//             if (jacobians[7])
//             {
//                 Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> jac(jacobians[7]);
//                 jac.setZero();
//                 jac.block<3,3>(param_ptr_->VEL_INDEX_STATE_, 0) = Qi.inverse().toRotationMatrix();
//                 jac = sqrt_info * jac;
//             }
...
//             if (jacobians[8])
//             {
//                 Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> jac(jacobians[8]);
//                 jac.setZero();
//                 jac.block<3,3>(param_ptr_->ACC_BIAS_INDEX_STATE_, 0) = Eigen::Matrix3d::Identity();
//                 jac = sqrt_info * jac;
//             }
...
//             if (jacobians[9])
//             {
//                 Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> jac(jacobians[9]);
//                 jac.setZero();
//                 jac.block<3,3>(param_ptr_->GYRO_BIAS_INDEX_STATE_, 0) = Eigen::Matrix3d::Identity();
//                 jac = sqrt_info * jac;
//             }
