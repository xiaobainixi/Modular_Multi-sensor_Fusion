...
{
    // 参考论文
    Vector3d g0 = g.normalized() * param_ptr->gw_.norm();
    Vector3d lx, ly;
    //VectorXd x;
    ...
    for(int k = 0; k < 4; k++)
    {
        ...
        for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
        {
            ...
            tmp_b.block<3, 1>(0, 0) =
                ...
                frame_i->second.R.transpose() * frame_j->second.R * param_ptr->tbc_ - param_ptr->tbc_ -
                frame_i->second.R.transpose() * dt * dt / 2 * g0;

            tmp_A.block<3, 3>(3, 0) = -Matrix3d::Identity();
            tmp_A.block<3, 3>(3, 3) = frame_i->second.R.transpose() * frame_j->second.R;
            tmp_A.block<3, 2>(3, 6) = frame_i->second.R.transpose() * dt * Matrix3d::Identity() * lxly;
            tmp_b.block<3, 1>(3, 0) = frame_j->second.pre_integration->delta_v_ - frame_i->second.R.transpose() * dt * Matrix3d::Identity() * g0;
    ...
    map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    // 做一些检查
    if(fabs(g.norm() - param_ptr->gw_.norm()) > 1.0 || s < 0)
    {
        return false;
    }
    // 重力修复
