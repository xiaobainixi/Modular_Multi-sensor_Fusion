...
PoseFactor::PoseFactor(const std::vector<double> anchor_value)
{
    for (int i = 0; i < 7; ++i)     _anchor_point(i) = anchor_value[i];
}

bool PoseFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
