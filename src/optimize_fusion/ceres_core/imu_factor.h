...
class IMUFactor : public ceres::SizedCostFunction<15, 7, 9, 7, 9>
{
  public:
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        // TODO : debug，将GNSS的误差设置为比较大的值
        // pre_integration->covariance = Eigen::Matrix<double, 15, 15>::Identity() * 1e-6;
        ...
        if (jacobians)
        {
            ...
            if (jacobians[0])
            {
                ...
                jacobian_pose_i.block<3, 3>(O_P, O_R) = Utility::skewSymmetric(Qi.inverse() * (0.5 * pre_integration->config.G * sum_dt * sum_dt + Pj - Pi - Vi * sum_dt));
            ...
            jacobian_pose_i.block<3, 3>(O_R, O_R) = -(Qj.inverse() * Qi).toRotationMatrix();
                ...
                jacobian_pose_i.block<3, 3>(O_V, O_R) = Utility::skewSymmetric(Qi.inverse() * (pre_integration->config.G * sum_dt + Vj - Vi));

                jacobian_pose_i = sqrt_info * jacobian_pose_i;

                if (jacobian_pose_i.maxCoeff() > 1e8 || jacobian_pose_i.minCoeff() < -1e8)
                {
                    std::cout << "numerical unstable in preintegration (sqrt_info)" << std::endl;
